...
struct UserProfileView: View {
    ...
    @StateObject private var reportBlockVM = ReportBlockViewModel()
    @StateObject private var requestVM = RequestViewModel()
    ...
    var body: some View {
        GeometryReader { geometry in
                        ...
                        HStack(spacing: 20) {
                            ...
                            case .received:
                                HStack(spacing: 20) {
                                    Button {
                                        Task {
                                            await requestVM.acceptRequest(currentUserId: session.uid, from: user.id)
                                        }
                                    } label: {
                                        VStack(spacing: 4) {
                                            Image(systemName: "checkmark.circle.fill")
                                    ...
                                    Button {
                                        Task {
                                            await requestVM.declineRequest(currentUserId: session.uid, from: user.id)
                                        }
                                    } label: {
                                        VStack(spacing: 4) {
                                            Image(systemName: "xmark.circle.fill")
            ...
            .confirmationDialog("More options", isPresented: $showActionMenu, titleVisibility: .visible) {
                if isBlocked {
                    Button("Unblock", role: .destructive) {
                        Task {
                            await reportBlockVM.unblockUser(targetId: user.id)
                            isBlocked = false
                        }
                    }
                } else {
                    Button("Block", role: .destructive) {
                        Task {
                            await reportBlockVM.blockUser(targetId: user.id)
                            isBlocked = true
                            toastMessage = "User blocked"
                            showToast = true
                            appRouter.goToExplore()
        ...
        .onAppear {
            Task {
                await requestVM.checkStatus(currentUserId: session.uid, viewedUserId: user.id)
                isBlocked = await reportBlockVM.isBlockedBetween(currentId: session.uid, targetId: user.id)
            }
        }
    }
